...
- **Direct input updates position synchronously.** Keyboard (arrows, PageUp/PageDown, Home/End on the focused feed) and touch drags update position without waiting on worker results. A single-touch flick continues after release with bounded, time-based exponential decay derived from recent touch samples. New direct input cancels that momentum, the exact position clamp remains authoritative at both ends, and reduced-motion preference keeps touch movement one-to-one.
...
- Touch scrolling has dependency-free momentum in the virtual-position input boundary. It adds no approximation to deck addressing: each animation frame advances the existing exact `FeedPosition`, and start/end checks plus the existing clamp stop motion at the finite boundaries.
...
- E2e gates: `e2e/explorer-home.spec.ts` (single-surface wheel and touch continuity with no document scroll), `e2e/explorer-fling.spec.ts` (exact full-distance wheel sequence and eventual row resolution), `e2e/explorer-momentum.spec.ts` (post-release continuation, cancellation, reduced motion, and end clamping), `e2e/explorer-rail.spec.ts` (visible end-to-end drag), `e2e/explorer-jump.spec.ts` (exact landing, reduced-motion path, cancellation, resize re-clamping, modified-wheel preservation, keyboard controls), and `e2e/explorer-end.spec.ts` (visible end-of-space regression guards).
