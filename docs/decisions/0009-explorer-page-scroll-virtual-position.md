...
- **Full-viewport virtual feed.** The feed fills the viewport and owns wheel and touch input without awaiting worker completion; the rendered strip (visible rows plus overscan) follows with a compositor transform. On the explorer-first home page, native document scrolling carries the compact introduction away before handing input to the feed, and receives upward input again when the feed returns to deck 1. Deck numbers derive synchronously from the position, while card faces remain asynchronous and main-thread rendering still determines end-to-end responsiveness.
...
- **Animated navigation.** Jump, Random, and Go-to-start/end interpolate the position with `requestAnimationFrame` over 300 ms (ease-in-out cubic). Mid-space targets land at the viewport top; an end target is exactly visible with the final deck pinned to the bottom row. The target is re-clamped if viewport geometry changes during the animation. `prefers-reduced-motion` (or a missing `matchMedia`) jumps instantly. Any new feed input — wheel, rail, keyboard, touch, or another navigation — cancels a running animation deterministically. A `data-animating` attribute exposes animation state to browser tests.
...
- The explorer lives at `/`; legacy `/explore` links redirect while preserving their query parameters. Decision 0012 defines the route and document-to-feed handoff without changing this decision's virtual-position authority.
...
- E2e gates: `e2e/explorer-home.spec.ts` (document/feed wheel and touch handoff), `e2e/explorer-fling.spec.ts` (exact full-distance wheel sequence and eventual row resolution), `e2e/explorer-rail.spec.ts` (visible end-to-end drag), `e2e/explorer-jump.spec.ts` (exact landing, reduced-motion path, cancellation, resize re-clamping, modified-wheel preservation, keyboard controls), and `e2e/explorer-end.spec.ts` (visible end-of-space regression guards).
