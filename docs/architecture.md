...
Mouse and pen dragging reorder live. Touch arbitrates among tap-to-move, manual horizontal panning, and long-press dragging; keyboard activation uses the same select-then-insert buttons. A platform-independent rejection-sampling algorithm draws an unbiased target from injected bytes, while `src/platform/crypto-entropy.ts` supplies Web Crypto in production. The UI animates old card slots to the exact target with the Web Animations API, handles cancellation, and settles immediately for reduced motion. No explorer worker, HTML drag-and-drop package, or motion package participates. Decision 0013 records the durable interaction model.
